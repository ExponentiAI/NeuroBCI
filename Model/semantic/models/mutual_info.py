...
class Mine(nn.Module):      #MI Model：P8 A。Simulation Settings
...
def mutual_information(joint, marginal, mine_net):      #公式9
    t = mine_net(joint)
    et = torch.exp(mine_net(marginal))
    mi_lb = torch.mean(t) - torch.log(torch.mean(et))       #公式9
    return mi_lb, t, et
...
def sample_batch(rec, noise):
    rec = torch.reshape(rec, shape=(-1, 1))     # reshape：举例 (2,5) -> (10,1)，（3，4）->(12,1)
    noise = torch.reshape(noise, shape=(-1, 1))
    rec_sample1, rec_sample2 = torch.split(rec, int(rec.shape[0]/2), dim=0)     #在第一个维度那里把rec分成两半：https://blog.csdn.net/foneone/article/details/103875250
